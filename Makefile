...
zip_linux:
	...
	docker run -v `pwd`:`pwd` -w `pwd` --rm swift:5.3.1-centos7 sh -c 'yum update -y && yum install zip libxml2-devel libcurl-devel -y && swift build -c release -Xswiftc -static-stdlib'
